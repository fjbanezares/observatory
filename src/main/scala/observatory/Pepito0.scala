...
*/
  ...
  val dfTemp = sqlContext.read.format("com.databricks.spark.csv").schema(customSchemaTemp).load(new File("src/main/resources/1977.csv").toURI.toString).na.fill("0",Seq("STN","WBAN"))
  //val dfTemp = spark.readStream.format("csv").schema(customSchemaTemp).csv("src/main/resources/1977.csv").na.fill("0",Seq("STN","WBAN"))
 ...
 //val dfStations = sqlContext.read.option("header","false").format("csv").schema(customSchemaStations).csv("src/main/resources/stations.csv")
